# 🌐 My Netlify Sites & Apps

❗[Türkçe README.md dosyası](https://github.com/cemal201138/My_Netlify_SitesApps/blob/main/README.md)❗

This repository contains a collection of lightweight web applications and demo projects built with **HTML, CSS, and JavaScript**, all hosted on Netlify.

Each project is organized within its own directory and deployed independently.

---

## 🚀 Live Applications

Below is the list of active projects currently deployed on Netlify:

### 🔹 Project List

- 📄 **Password Decryptor (Şifre Çöz)**

  👉 Link: https://cemalevli.qzz.io/Şifre_Çöz/
  
- 📄 **Personal Portfolio**
  
  👉 Link: https://cemalevli.qzz.io/Portfolio_Site/

- 📄 **Music Control**
  
  👉 Link: https://cemalevli.qzz.io/Müzik_Kontrol/

- 📄 **MacroDroid Media Control**
 
  👉 Link: https://cemalevli.qzz.io/MacroDroid_Media_Control/

- 📄 **Audio Hub (Ses Sitesi)**
  
  👉 Link: https://cemalevli.qzz.io/ses_sitesi/

- **! Other projects are currently in the testing phase and are not yet listed !**


> 🔔 This list is updated periodically as new projects are added.

---

## 📜 License

These projects are created for educational and demonstration purposes.  
You are free to use them by providing proper attribution to the source.

---
